...
*/
int PFF_read_file(char * dest, int to_read, int * read_from)
{
...
*/
void PFF_setup_stream(void (* pre_block)(void), void (* pre_byte)(void), char (* dest)(char), void (* post_byte)(void), void (* post_block)(void))
{
...
*/
int PFF_stream_file(int to_read, int * read_from)
{
...
int PFF_up_dir()
{
	...
	if (i >= 1) // if not already in root
	{
		...
		res = PFF_open_dir(path); // attempt to open
		free(path); // free string
	}
	else
	{
		res = PFF_open_dir((char *)"/"); // reopen root
	}
	return res;
}
...
int PFF_open(FILINFO * fnfo)
{
	...
	{
		res = PFF_open_dir(fpath); // open directory
	}
	else // is a file
	{
